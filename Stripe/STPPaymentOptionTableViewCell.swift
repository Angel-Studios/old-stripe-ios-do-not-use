...
class STPPaymentOptionTableViewCell: UITableViewCell {
  ...
  override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
    ...
    NSLayoutConstraint.activate(
      [
        ...
        self.leftIcon.centerYAnchor.constraint(lessThanOrEqualTo: contentView.centerYAnchor),
    ...
    if let paymentOption = paymentOption as? STPCard {
      ...
      if paymentMethod.type == .FPX && paymentMethod.fpx != nil {
        return buildAttributedString(
          with: STPFPXBank.brandFrom(paymentMethod.fpx?.bankIdentifierCode), selected: selected)
      }
      ...
      if paymentMethodParams.type == .FPX && paymentMethodParams.fpx != nil {
        return buildAttributedString(
          with: paymentMethodParams.fpx?.bank ?? STPFPXBankBrand.unknown, selected: selected)
      }
    }
  ...
  func buildAttributedString(withCardPaymentMethod paymentMethod: STPPaymentMethod, selected: Bool)
    -> NSAttributedString
  {
    return buildAttributedString(
  ...
  func buildAttributedString(with bankBrand: STPFPXBankBrand, selected: Bool) -> NSAttributedString
  {

    ...
    set(autoFormattingBehavior) {
      ...
      case .cardNumbers:
        textFormattingBlock = { inputString in
          ...
          for segmentLength in cardNumberFormat {
            ...
            while index < (attributedString.length) && segmentIndex < Int(segmentLength.uintValue) {
      ...
      case .phoneNumbers:
        ...
        textFormattingBlock = { inputString in
          ...
          return NSAttributedString(
            string: phoneNumber, attributes: attributes as? [NSAttributedString.Key: Any])
        }
      case .bsbNumber:
        weak var weakSelf = self
        textFormattingBlock = { inputString in
          ...
          let bsbNumber = STPBSBNumberValidator.formattedSanitizedText(from: inputString.string)
          let attributes = type(of: strongSelf).attributes(for: inputString)
          return NSAttributedString(
            string: bsbNumber ?? "", attributes: attributes as? [NSAttributedString.Key: Any])
        }
      }
    }
  }
...
class STPTextFieldDelegateProxy: NSObject, UITextFieldDelegate {
  ...
  func textField(
    ...
    let deleting =
      range.location == (textField.text?.count ?? 0) - 1 && range.length == 1 && (string == "")

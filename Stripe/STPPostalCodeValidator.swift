...
class STPPostalCodeValidator: NSObject {
  class func postalCodeIsRequired(forCountryCode countryCode: String?) -> Bool {
    if countryCode == nil {
      ...
      return
        !(self.countriesWithNoPostalCodes()?.contains(countryCode?.uppercased() ?? "") ?? false)
    }
  }
  ...
  class func validationState(forUSPostalCode postalCode: String?) -> STPCardValidationState {
    ...
    if !firstFiveIsNumeric {
      ...
      if numberOfDigits > 9 {
        ...
        if countOfCharactersFromSetInString(separatorCharacter ?? "", CharacterSet.stp_asciiDigit)
          == 0
        {
  ...
  class func formattedSanitizedUSZipCode(
    ...
    var formattedString = STPCardValidator.sanitizedNumericString(for: zipCode).stp_safeSubstring(
      to: maxLength)

...
public class STPSource: NSObject, STPAPIResponseDecodable, STPSourceProtocol, STPPaymentOption {
  ...
  class func string(from flow: STPSourceFlow) -> String? {
    return
      (self.stringToFlowMapping() as NSDictionary).allKeys(for: NSNumber(value: flow.rawValue))
      .first as? String
  }
  ...
  class func string(from usage: STPSourceUsage) -> String? {
    return
      (self.stringToUsageMapping() as NSDictionary).allKeys(for: NSNumber(value: usage.rawValue))
      .first as? String
  }
  ...
  public class func decodedObject(fromAPIResponse response: [AnyHashable: Any]?) -> Self? {
    ...
    if let rawVerification = rawVerification {
      source.verification = STPSourceVerification.decodedObject(fromAPIResponse: rawVerification)
    }
